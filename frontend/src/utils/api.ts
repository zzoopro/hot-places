...
export const getPlaces = async (offset?: number) => {
  return await axios(`${BASE_SERVER_URL}/api/places/all`, AuthHeader).then(
    (response: any) => response
  );
};
export const getPlace = async (id: string) => {
  return await axios(`${BASE_SERVER_URL}/api/places/${id}`, AuthHeader).then(
    (response: any) => response
  );
};
...
export const postPlace = async (data: any) => {
  return await axios.post(
    `${BASE_SERVER_URL}/api/places/add`,
    data,
    AuthHeader
  );
};
